...
parser.add_option("-i", dest="rnaseq_results",
                  default="mouse_LOGFC_1.50_FDR_0.01",
...
###########################################################################
    #interogate_random("phydis_head_expressed", "phydis_LOGFC_1.50_FDR_0.01")
    ...
    f_in = open("phydis_head_expressed", "r")
    all_genes = []
    for line in f_in:
        line = line.upper()
    ...
    rnaseq = open("phydis_LOGFC_1.50_FDR_0.01", "r")
    DE_set = set([])
    for line in rnaseq:
        data = line.split()
        gene = data[0]
